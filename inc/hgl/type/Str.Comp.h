...
namespace hgl
{
    ...
    /**
     * @brief
     * CN: 比较两个以 NUL 结尾的字符串的大小，返回 std::strong_ordering（逐字符比较）。
     * EN: Compare two NUL-terminated strings lexicographically, returning std::strong_ordering.
     *
     * @tparam S  源字符串类型 / source string character type
     * @tparam D  目标字符串类型 / destination string character type
     * @param  src       CN: 要比较的字符串 / source string to compare
     ...
     * @param  dst       CN: 要比较的字符串 / destination string to compare
     *                    EN: pointer to the second string (may be nullptr)
     * @return           CN: 返回 std::strong_ordering，表示 src 与 dst 的比较结果
     *                    EN: Returns std::strong_ordering indicating comparison result
     */
    template<typename S,typename D>
    inline std::strong_ordering strcmp_ordering(const S *src,const D *dst)
    {
        if(!src)
        ...
        // If both ended, equal; otherwise shorter is less
        if(*src) return std::strong_ordering::greater;
        if(*dst) return std::strong_ordering::less;
     ...
     * @brief
     * CN: 比较两个以 NUL 结尾的字符串的大小，返回 std::strong_ordering（逐字符比较）。
     * EN: Compare two strings with specified lengths lexicographically, returning std::strong_ordering.
     *
     * @tparam S  源字符串类型 / source string character type
     * @tparam D  目标字符串类型 / destination string character type
     * @param  src       CN: 要比较的字符串1 / first string pointer
     ...
     */
    template<typename S,typename D>
    inline std::strong_ordering strcmp_ordering(const S *src, std::size_t src_size, const D *dst, std::size_t dst_size)
    {
        if(!src)
            return (!dst) ? std::strong_ordering::equal : std::strong_ordering::less;
        ...
        if(src_size==0)
            return (dst_size==0) ? std::strong_ordering::equal : std::strong_ordering::less;
        ...
        if(src_size>dst_size) return std::strong_ordering::greater;
        if(src_size<dst_size) return std::strong_ordering::less;
        return std::strong_ordering::equal;
    }
    ...
    /**
     * @brief
     * CN: 比较两个以 NUL 结尾的字符串的大小，返回 std::strong_ordering（限制最大比较长度范围内逐字符比较）。
     * EN: Compare two strings with a maximum comparison length lexicographically, returning std::strong_ordering.
     *
     * @tparam S  源字符串类型 / source string character type
     * @tparam D  目标字符串类型 / destination string character type
     * @param  src       CN: 要比较的字符串 / source string to compare
     ...
     */
    template<typename S,typename D>
    inline std::strong_ordering strcmp_ordering(const S *src,const D *dst,std::size_t count)
    {
        if(count==0) return std::strong_ordering::equal;

        if(!src) return (!dst) ? std::strong_ordering::equal : std::strong_ordering::less;
        if(!dst) return std::strong_ordering::greater;
    ...
    /**
     * @brief
     * CN: 比较两个字符串的大小（限制最大比较长度），英文字符不区分大小写（case-insensitive for ASCII letters）。
     * EN: Compare two strings with a maximum comparison length case-insensitively for ASCII letters。
     *
     * @tparam S  源字符串类型 / source string character type
     * @tparam D  目标字符串类型 / destination string character type
     * @param  src       CN: 要比较的字符串 / source string pointer
     ...
     * @param  count     CN: 比较的最大长度 / maximum number of characters to compare (must be >= 0)
     *                    EN: maximum comparison length (size_t)
     * @return           CN: 返回 -1/0/+1 分别表示 src < dst、src == dst（在 count 范围内）、src > dst（不区分大小写）
     *                    EN: Returns -1/0/+1 for src < dst, src == dst (within count), src > dst (case-insensitive)
     */
    template<typename S,typename D>
    ...
    /**
     * @brief
     * CN: 比较两个以 NUL 结尾的字符串的大小，英文字符不区分大小写（case-insensitive for ASCII letters）。
     * EN: Compare two NUL-terminated strings case-insensitively for ASCII letters。
     *
     * @tparam S  源字符串类型 / source string character type
     * @tparam D  目标字符串类型 / destination string character type
     * @param  src       CN: 要比较的字符串 / source string to compare
     ...
     */
    template<typename S,typename D>
    inline std::strong_ordering stricmp_ordering(const S *src,const D *dst)
    {
        if(!src)
            return (!dst) ? std::strong_ordering::equal : std::strong_ordering::less;
    ...
    /**
     * @brief
     * CN: 比较两个以 NUL 结尾的字符串的大小，英文字符不区分大小写，返回 std::strong_ordering（限制最大比较长度范围内逐字符比较）。
     * EN: Compare two strings with a maximum comparison length lexicographically, returning std::strong_ordering, case-insensitively.
     *
     * @tparam S  源字符串类型 / source string character type
     * @tparam D  目标字符串类型 / destination string character type
     * @param  src       CN: 要比较的字符串 / source string to compare
     ...
     */
    template<typename S,typename D>
    inline std::strong_ordering stricmp_ordering(const S *src,const D *dst,std::size_t count)
    {
        if(count==0) return std::strong_ordering::equal;
